...
| **Phase 4** — Nav2 + exploration | 🟢 Working (final demo polish pending). `nav2.launch.py` is a **minimal** bringup (controller/planner/behaviors/bt + own lifecycle_manager — avoids `nav2_bringup`'s `collision_monitor` abort; controller publishes Twist straight to `/cmd_vel`). Verified: Nav2 **activates, accepts a `NavigateToPose` goal, plans, and the controller reaches goal-tolerance**. Costmaps build from `/map`+`/scan`. **Map-first matters:** plan only once the robot has mapped its area (else "robot out of bounds of costmap") — drive/`frontier_explorer` first. `frontier_explorer` frontier detection unit-tested. |
...
| **Phase 6** — Task executive | 🟢 Working (loop logic verified; live integration ready). `spot_sar_executive/task_executive`: SENSE→GROUND→PLAN→ACT→MONITOR→REPLAN — reads `/world_model`, builds+solves a PDDL problem, dispatches the first action (move/explore→`NavigateToPose` — which Nav2 now accepts, report→logged), replans each cycle. Loop logic verified end-to-end (incl. replan shortening on arrival). |
| Phase 7 | 🟡 Evaluation plan in [docs/Phase7_Evaluation.md](docs/); metrics pending a polished demo run. |
| Full stack | `ros2 launch spot_sar_bringup sar_system.launch.py` + `world_model_node` + `task_executive` (planning venv). All components verified individually + SLAM/Nav2 up together; the full *simultaneous* stack (Isaac RTX + SLAM + Nav2 + detector + brain) is **memory-tight on 8 GB VRAM** — a node can be OOM-killed under peak. For a polished demo: fresh boot, explore-to-map first, then run the executive; or offload the ROS-side stack to the `unige_legged` container on a roomier host. |
